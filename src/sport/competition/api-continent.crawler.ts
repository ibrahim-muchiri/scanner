...
@Injectable()
export class ApiContinentCrawler implements Crawler {
  private readonly logger = new Logger(ApiContinentCrawler.name);
  ...
  async run() {
    ...
    const data = await this.sportApi.fetchAllContinents();
    await this.bettles.updateContinents(data);
  ...
  async runOnce() {
    ...
    if (this.config.persistAsJson) {
      this.logger.verbose(`Persist JSON files to file system for continents`);
      const outputFilename = `${this.config.mockDbPath}/_all/continents.json`;
